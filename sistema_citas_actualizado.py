...
st.image(direccion_logo, width=600)  # Aumentado de 400 a 600
...
def guardar_en_supabase(datos):
    try:
        ...
        cursor = conn.cursor()
        cursor.execute("""
            CREATE TABLE IF NOT EXISTS citas_ingauto (
...
def exportar_citas_csv():
    try:
        conn = conectar_supabase()
        cursor = conn.cursor()
        cursor.execute("SELECT * FROM citas_ingauto ORDER BY id DESC")
        filas = cursor.fetchall()
        columnas = [desc[0] for desc in cursor.description]
        df = pd.DataFrame(filas, columns=columnas)
        df.insert(0, "#", range(1, 1 + len(df)))  # Enumerar citas
        conn.close()
...
def generar_pdf(datos, nombre_archivo):
    ...
    pdf.image(direccion_logo, x=10, y=8, w=80)  # Logo más grande
    pdf.ln(30)
